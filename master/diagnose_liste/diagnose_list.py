...
import time
import pandas as pd
from global_functions import move_mouse_on, select_and_copy_data_from_table, save_data_to_excel, \
...
def click_search_button():
    ...
    time.sleep(1)
    click_by_mouse_on('master/images/vitale/01-search.jpg', confidence=0.95)
    time.sleep(1)
...
def extract_table_data():
    ...
    :rtype: str
    """
    return select_and_copy_data_from_table('master/images/diagnose_list/02-top-left-corner.jpg',
                                           'master/images/diagnose_list/03-top-right-corner.jpg',
                                           'master/images/diagnose_list/04-bottom-right-corner.jpg',
...
def main_extract_diagnose_list_data(path_to_save_data: str) -> None:
    ...
    try:
        ...
        click_by_mouse_on('master/images/diagnose_list/00-scroll-down.jpg', confidence=0.7)
        time.sleep(3)

        move_mouse_on('master/images/diagnose_list/01-diagnoseoverblik.jpg')
        time.sleep(3)
        ...
        if len(df) > 0:
           ...
           write_dataframe_to_excel(df, os.path.join(path_to_save_data, 'diagnose_list.xlsx'))
        else:
            print(f"No Diagnose List data exist for: {path_to_save_data}")
        ...
        import traceback
        _, _, tb = sys.exc_info()
        print(f"Failed to extract Diagnose List data: {path_to_save_data} \n  {e.args[0]} Line: {tb.tb_lineno}")
        traceback.print_exc()