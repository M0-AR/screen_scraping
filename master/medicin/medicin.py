...
import pyautogui
import pyperclip
from global_functions import click_by_mouse_on, move_mouse_on, write_dataframe_to_excel
...
def main_extract_medicin_data(path_to_save_data):
    ...
    try:
        ...
        click_by_mouse_on('master/images/medicin/01-medicin.jpg')
        time.sleep(2)

        # Click on the second image and wait for 2 seconds
        click_by_mouse_on('master/images/medicin/02-unselect.jpg')
        time.sleep(2)

        # Click on the third image, move 60 pixels on the x-axis, and wait for 1 second
        click_by_mouse_on('master/images/medicin/03-flere.jpg', move_x=27, confidence=0.9)
        time.sleep(1)

        # Click on the fourth image and wait for 1 second
        click_by_mouse_on('master/images/medicin/04-select-all.jpg')
        time.sleep(1)

        # Move the mouse to the position of the fifth image and wait for 1 second
        move_mouse_on('master/images/medicin/05-right-click.jpg', confidence=0.8)
        time.sleep(1)
        ...
        click_by_mouse_on('master/images/medicin/06-show-result-in-side-panel.jpg')
        time.sleep(2)

        # Click on the seventh image and wait for seconds
        click_by_mouse_on('master/images/medicin/07-copy-all.jpg', confidence=0.90)
        time.sleep(15)
        ...
        if len(df) > 0:
            ...
            print(f"No medicin data exist for: {path_to_save_data}")
