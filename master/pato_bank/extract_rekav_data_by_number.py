...
from global_functions import click_by_mouse_on, select_and_copy_data_from_table, write_dataframe_to_excel, lev_dist, remove_existing_file

REKV_NUM_PATTERNS = [
    r'\b[0-9]{8}\b',
    r'\b[0-9]{2}[a-zA-Z]{2}[0-9]{4}\b',
    r'\b[0-9]{2}[a-zA-Z]{2}[0-9]{5}\b',
...
def capture_screenshot(cursor_position: Tuple[int, int], width: int = 100, height: int = 100):
    ...
    screenshot_image.save('test.png')
...
def find_number_in_screenshot(pattern, screenshot):
    ...
    for i in range(n_boxes):
        ...
        # Calculate the Levenshtein distance between the text and pattern
        lev_distance = lev_dist(pattern, text)

        # Calculate the similarity as a percentage
        similarity = 1 - (lev_distance / max(len(pattern), len(text)))
...
def click_pato_bank():
    ...
    """
    click_by_mouse_on('master/images/pato_bank/01-patobank.jpg')
    time.sleep(2)
...
def extract_table_text():
    ...
    return select_and_copy_data_from_table(
        up_left_corner_position='master/images/pato_bank/02-cgi-logo.jpg',
        up_right_corner_position='master/images/pato_bank/03-end-of-selection.jpg',
        end_scroll_position='master/images/pato_bank/04-scroll-down-until-sign-found.jpg',
        scroll_if_sign_found='master/images/general/scroll-bar-pato-bank.jpg',
...
def find_and_move_to_rekv_column():
    ...
    """
    rekv_location = pyautogui.locateCenterOnScreen('master/images/pato_bank/05-rekv-column.jpg', confidence=0.85)
    ...
    if rekv_number is not None:
        ...
        if number_bbox is not None:
            ...
            rekv_number_data = select_and_copy_data_from_table(
                up_left_corner_position='master/images/pato_bank/06-top_left_selection-rekv-nr.jpg',
                up_right_corner_position='master/images/pato_bank/07-top-right-selection-rekv-nr.jpg',
                end_scroll_position='master/images/pato_bank/08-end-of-scroll-rekv-nr.jpg',
                scroll_if_sign_found='master/images/general/scroll-bar-pato-bank.jpg',
                confidence_of_end_scroll=0.8,
                move_y_end_scroll=50
            )
        ...
        if pyautogui.locateOnScreen('master/images/pato_bank/04-scroll-down-until-sign-found.jpg', confidence=0.8):
            return False
    return None
...
def main_extract_pato_bank_data(path_to_save_data: str) -> None:
    ...
    try:
        ...
        pytesseract.pytesseract.tesseract_cmd = r"C:\Users\mashm\AppData\Local\Programs\Tesseract-OCR\tesseract.exe"
        ...
        for target_number in sorted_rekv_nrs:
            ...
            while not found_number:
                ...
                if rekv_number_data == False:
                    import logging
                    logging.basicConfig(filename='master/log/logger.log', level=logging.DEBUG)
                    logging.debug(f"\nFailed to extract {target_number} data: {path_to_save_data} \n")
                    break
        ...
        if len(df) > 0:
            ...
            print(f"No Pato Bank data exist for: {path_to_save_data}")
        ...
        import traceback
        _, _, tb = sys.exc_info()
        print(f"Failed to extract Pato Bank data: {path_to_save_data} \n  {e.args[0]} Line: {tb.tb_lineno}")
        traceback.print_exc()